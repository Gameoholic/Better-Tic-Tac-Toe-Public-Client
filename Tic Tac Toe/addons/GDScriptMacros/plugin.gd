...
func _init_macro_file() -> void:
	var file0 := File.new()
	var date := file0.get_modified_time(macroPath)
...
func _on_gui_focus_changed(node: Node):
	if node is TextEdit:
		if is_instance_valid(script_editor):
			script_editor.disconnect("text_changed", self,"_on_caret_changed")
		script_editor = node
		if !script_editor.is_connected("text_changed", self, "_on_caret_changed"):
			script_editor.connect("text_changed", self, "_on_caret_changed")
